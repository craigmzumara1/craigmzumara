...
const ADMIN_USERNAME = process.env.ADMIN_USERNAME || "craigmzumara1";
 ...
 * Production Firebase -> vercel requests use Bearer JWTs.
 */
...
function adminAuth(req, res, next) {
  ...
  if (authHeader.startsWith("Bearer ")) {
    const payload = verifyJwt(authHeader.slice("Bearer ".length).trim());
...
async function adminLogin(req, res) {
  try {
    ...
    const expiresIn = parseDuration(TOKEN_EXPIRES_IN);
