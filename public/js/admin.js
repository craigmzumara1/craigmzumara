...
const ADMIN_TOKEN_KEY = "craigmzumara_admin_token";
...
async function loginAdmin() {
  ...
  adminLoginPromise = new Promise((resolve) => {
    ...
    const username = document.getElementById("admin-auth-username");
    ...
    const submit = async (event) => {
      ...
      try {
        ...
        if (!response.ok || !result.success || !result.token) {
          throw new Error(
            result.error || `Authentication failed (HTTP ${response.status}).`,
          );
        }
...
async function authenticatedRequest(url, options = {}, retry = true) {
  ...
  try {
    ...
    console.error("vercel API network/CORS error:", error);

    throw new Error(
      "Could not communicate with the vercel server. Check the vercel API and CORS configuration.",
    );
  }

  if (response.status === 401 && !IS_LOCAL_API && retry) {
    clearAdminToken();
    await loginAdmin();
...
function initializeImagePreviews() {
  document.querySelectorAll(".file-input").forEach((input) => {
    input.addEventListener("change", (event) => {
      ...
      const targetImgId = event.target.getAttribute("data-target-img");
      ...
      if (event.target.id === "file-blog-image") {
...
async function initializeBlogCMS() {
  const form = document.getElementById("blog-publish-form");
  ...
  const resetButton = document.getElementById("blog-reset-button");

  /*
   * Load categories and tags.
   */

  await loadBlogCategories(categorySelect);

  await loadBlogTags(tagsContainer);
  ...
  if (statusSelect) {
    statusSelect.addEventListener("change", () => {
      submitText.textContent =
        statusSelect.value === "draft" ? "Save Draft" : "Publish Post";
    });
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const imageInput = document.getElementById("file-blog-image");
    ...
    const selectedTags = Array.from(
      document.querySelectorAll('input[name="blog-tag"]:checked'),
    ...
    submitText.textContent = status === "draft" ? "Saving..." : "Publishing...";
    ...
    try {
      ...
      alert(error.message || "Failed to save the blog post.");
...
async function loadBlogCategories(select) {
  if (!select) return;
...
async function loadBlogTags(container) {
  if (!container) return;
  ...
  try {
    ...
    listContainer.innerHTML = posts
      .map((post) => {
        ...
        const status = escapeHtml(post.status || "published");

        const category = escapeHtml(post.category_name || "Uncategorized");
...
async function deleteBlogPost(postId) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        result.error || `Delete failed with status ${response.status}.`,
      );
    }
    ...
    const element = document.getElementById(`admin-post-${postId}`);
...
async function loadAdminImages() {
  try {
    ...
    data.forEach((item) => {
      ...
      const subtitleInput = form.querySelector(".subtitle-input");

      const badgeInput = form.querySelector(".badge-input");

      const techTagsInput = form.querySelector(".tech-tags-input");

      const demoUrlInput = form.querySelector(".demo-url-input");

      const githubUrlInput = form.querySelector(".github-url-input");

      if (titleInput && item.title) {
        titleInput.value = item.title;
      }

      if (subtitleInput && item.subtitle) {
        subtitleInput.value = item.subtitle;
      }

      if (badgeInput && item.badge) {
        badgeInput.value = item.badge;
      }

      if (techTagsInput && item.tech_tags) {
        techTagsInput.value = item.tech_tags;
      }

      if (demoUrlInput && item.live_demo_url) {
        demoUrlInput.value = item.live_demo_url;
      }

      if (githubUrlInput && item.github_url) {
        githubUrlInput.value = item.github_url;
      }
    });
  } catch (error) {
...
function initializeProjectAndPhotoForms() {
  ...
  forms.forEach((form) => {
    form.addEventListener("submit", async (event) => {
      ...
      const elementId = form.getAttribute("data-element");

      const fileInput = form.querySelector(".file-input");

      const titleInput = form.querySelector(".title-input")?.value || "";

      const subtitleInput = form.querySelector(".subtitle-input")?.value || "";

      const badgeInput = form.querySelector(".badge-input")?.value || "";

      const techTagsInput = form.querySelector(".tech-tags-input")?.value || "";

      const demoUrlInput = form.querySelector(".demo-url-input")?.value || "";

      const githubUrlInput =
        form.querySelector(".github-url-input")?.value || "";
      ...
      formData.append("badge", badgeInput);

      formData.append("tech_tags", techTagsInput);

      formData.append("live_demo_url", demoUrlInput);

      formData.append("github_url", githubUrlInput);
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            result.error || `Update failed with status ${response.status}.`,
          );
        }
        ...
        alert(error.message || "Error updating data.");
      } finally {
        if (submitButton) {
          submitButton.disabled = false;
